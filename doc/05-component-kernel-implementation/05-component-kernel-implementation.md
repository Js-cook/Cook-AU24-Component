...
The representation will likely be a Sequence similar to how each coordinate is represented. While the ordering of the coordinates isn't particularly significant, the methods for Sequence will be useful in implementing the kernel methods of StatCalc which is why I selected it ultimately (in addition to dynamic sizing). There won't be any restrictions on the representation of the whole structure but once a dimensionSize is selected in a constructor that will limit the behavior of certain methods.
